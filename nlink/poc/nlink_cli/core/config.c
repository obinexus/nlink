...
nlink_pass_mode_t nlink_detect_pass_mode(const char* project_root_path) {
    ...
    snprintf(pkg_config_path, sizeof(pkg_config_path), "%s/pkg.nlink", project_root_path);
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_DIR && 
            ...
            snprintf(subdir_path, sizeof(subdir_path), "%s/%s", project_root_path, entry->d_name);
            
            // Check if subdirectory contains source files or nlink.txt
            char nlink_txt_path[NLINK_MAX_PATH_LENGTH];
            snprintf(nlink_txt_path, sizeof(nlink_txt_path), "%s/nlink.txt", subdir_path);
...
int nlink_discover_components(const char* project_root_path,
    ...
    while ((entry = readdir(dir)) != NULL && discovered_count < NLINK_MAX_COMPONENTS) {
        if (entry->d_type == DT_DIR && 
            strcmp(entry->d_name, ".") != 0 && 
            strcmp(entry->d_name, "..") != 0) {
...
nlink_config_result_t nlink_validate_config(const nlink_pkg_config_t* config) {
    ...
    if (config->pass_mode == NLINK_PASS_MODE_MULTI && config->component_count <= 1) {
        fprintf(stderr, "[CONFIG WARNING] Multi-pass mode configured but only %d components found\n", 
