 ...
 * Implements deterministic build mode resolution and threading pool configuration.
 * 
 * Architecture:
...
#define NLINK_CONFIG_LOCK() \
    do { \
        ...
        if (global) pthread_mutex_lock(&global->config_mutex); \
    } while(0)

#define NLINK_CONFIG_UNLOCK() \
    do { \
        nlink_global_config_t* global = nlink_get_global_config(); \
        if (global) pthread_mutex_unlock(&global->config_mutex); \
    } while(0)
