...
typedef struct {
    ...
    int minor;              // Minor version (backward compatible additions)
    int patch;              // Patch version (backward compatible fixes)
 ...
 */
typedef enum {
    NEXUS_VERSION_EQ,       // Equal to (=)
    NEXUS_VERSION_GT,       // Greater than (>)
    NEXUS_VERSION_GTE,      // Greater than or equal to (>=)
    NEXUS_VERSION_LT,       // Less than (<)
    NEXUS_VERSION_LTE,      // Less than or equal to (<=)
    NEXUS_VERSION_CARET,    // Compatible with (^) - same major version
    NEXUS_VERSION_TILDE,    // Compatible with (~) - same major.minor version
    NEXUS_VERSION_ANY       // Any version (*)
 ...
 */
typedef struct {
    NexusVersionOperator op;  // Comparison operator
...
NexusVersionConstraint* nexus_version_constraint_parse(const char* constraint_str) {
    ...
    // Default to equality
    constraint->op = NEXUS_VERSION_EQ;
    
    // Check for wildcard
    if (strcmp(constraint_str, "*") == 0 || strcmp(constraint_str, "latest") == 0) {
        constraint->op = NEXUS_VERSION_ANY;
    ...
    if (constraint_str[0] == '^') {
        constraint->op = NEXUS_VERSION_CARET;
        version_start = constraint_str + 1;
    } else if (constraint_str[0] == '~') {
        constraint->op = NEXUS_VERSION_TILDE;
        version_start = constraint_str + 1;
    } else if (constraint_str[0] == '>' && constraint_str[1] == '=') {
        constraint->op = NEXUS_VERSION_GTE;
        version_start = constraint_str + 2;
    } else if (constraint_str[0] == '<' && constraint_str[1] == '=') {
        constraint->op = NEXUS_VERSION_LTE;
        version_start = constraint_str + 2;
    } else if (constraint_str[0] == '>') {
        constraint->op = NEXUS_VERSION_GT;
        version_start = constraint_str + 1;
    } else if (constraint_str[0] == '<') {
        constraint->op = NEXUS_VERSION_LT;
        version_start = constraint_str + 1;
    } else if (constraint_str[0] == '=') {
        constraint->op = NEXUS_VERSION_EQ;
        version_start = constraint_str + 1;
...
bool nexus_version_satisfies(const NexusVersion* version, const NexusVersionConstraint* constraint) {
    ...
    if (constraint->op == NEXUS_VERSION_ANY) return true;
    
    // Compare according to operator
    int comparison = nexus_version_compare(version, constraint->version);
    
    switch (constraint->op) {
        case NEXUS_VERSION_EQ:
            return comparison == 0;
        
        case NEXUS_VERSION_GT:
            return comparison > 0;
        
        case NEXUS_VERSION_GTE:
            return comparison >= 0;
        
        case NEXUS_VERSION_LT:
            return comparison < 0;
        
        case NEXUS_VERSION_LTE:
            return comparison <= 0;
        
        case NEXUS_VERSION_CARET:
