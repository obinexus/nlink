 ...
 * 
 * This header defines macros and structures for implementing version-aware
 * lazy-loaded functions with support for versioned symbol resolution.
 * 
...
#include <string.h>
#include <time.h>
#include "symbols/nexus_versioned_symbols.h"
...
/**
 * @brief Enhanced macro for version-aware lazy-loading functions
 * 
 * This macro defines a lazy-loading function with version awareness.
 * 
 * @param func_name Name of the function
 * @param ret_type Return type of the function
 * @param version_constraint Version constraint string
 * @param ... Parameter list of the function
 */
...
#endif // NEXUS_LAZY_VERSIONED_Hwithout versioning */ \
        func_name##_impl = (func_name##_t)dlsym(func_name##_handle, #func_name); \
        if (!func_name##_impl) { \
          fprintf(stderr, "NexusLink symbol error: %s\n", dlerror()); \
          exit(1); \
