...
#include <nlink/core/etps/etps_telemetry.h>
#include <stdio.h>
// src/core/etps/etps_telemetry.c

#include <nlink/core/semverx/nlink_semver_core.h>  // <--- Fix this header
...
void etps_shutdown_legacy(void) {
    if (g_event_buffer) free(g_event_buffer);
    g_event_buffer = NULL;
    g_event_capacity = 0;
    g_event_count = 0;
...
etps_result_t etps_context_create(etps_context_t* ctx, const char* context_name) {
    if (!ctx) return -1;
...
void etps_context_destroy(etps_context_t* ctx) {
    if (!ctx) return;
    if (ctx->registered_components) free(ctx->registered_components);
    ctx->is_active = false;
    ctx->registered_components = NULL;
    ctx->component_count = 0;
}
...
const char* etps_range_state_to_string(semverx_range_state_t state) {
    switch (state) {
        case SEMVERX_RANGE_MATCH: return "MATCH";
        case SEMVERX_RANGE_MISMATCH: return "MISMATCH";
...
const char* etps_hotswap_result_to_string(hotswap_result_t result) {
    switch (result) {
        ...
        case HOTSWAP_FAILURE: return "FAILURE";
        default: return "UNKNOWN";
    }
}
...
int etps_register_component(etps_context_t* ctx, const semverx_component_t* component) {
    ...
    size_t new_capacity = ctx->component_count + 1;
    semverx_component_t* new_array = realloc(ctx->registered_components, new_capacity * sizeof(semverx_component_t));
    ...
    printf("[ETPS] Component Registered: %s %s [%s]\n",
...
static bool is_compatible_range_state(semverx_range_state_t source, semverx_range_state_t target, bool strict_mode) {
    if (strict_mode)
        return source == SEMVERX_RANGE_MATCH && target == SEMVERX_RANGE_MATCH;
    return source != SEMVERX_RANGE_MISMATCH && target != SEMVERX_RANGE_MISMATCH;
}
...
compatibility_result_t etps_validate_component_compatibility(
    ...
    if (!ctx || !source_component || !target_component || !event) return INCOMPATIBLE;
    bool compatible = is_compatible_range_state(source_component->range_state, target_component->range_state, true);
    if (!compatible) {
        strncpy(event->event_id, "compatibility_violation", sizeof(event->event_id));
...
void etps_emit_semverx_event(etps_context_t* ctx, const etps_semverx_event_t* event) {
    ...
    printf("[ETPS] SemVerX Event: %s\n", event->event_id);
    printf("  From: %s %s [%s]\n", event->source_component.name, event->source_component.version,
           etps_range_state_to_string(event->source_component.range_state));
    printf("  To  : %s %s [%s]\n", event->target_component.name, event->target_component.version,
